...
serve(async (req) => {
  ...
  try {
    const { gross_pay, pay_date, paye_tax_code, annual_gross, annualized_gross, role_tier } = await req.json();
    ...
    const annualGross = annual_gross || annualized_gross || gross_pay * 12;
    ...
    // Role Tier exemptions (Merge of HEAD logic)
    const bracket1Threshold = rateMap.get("paye_threshold")?.ceiling_amount || 1500096;
    let payeExempt = false;
    
    if (role_tier === "ict_tsr") {
      // Exempt from PAYE if annual income is below personal threshold
      if (annualGross <= bracket1Threshold) {
        payeExempt = true;
      }
    }

    // Adjust for PAYE tax code or exemptions
    if (payeExempt || paye_tax_code === "E") {
      payeAmount = 0; // Exempt
    ...
    console.error("[calculate-ja-statutory] Error:", error);
    return new Response(
